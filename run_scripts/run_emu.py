...
def run_google_scenarios(base_cmd):
    ...
    cmd = base_cmd + ' --id "%s" --desc "%s" --uplink-trace ~/traces/12mbps.trace --downlink-trace ~/traces/12mbps.trace --prepend-mm-cmds "mm-delay 10" --extra-mm-link-args "--uplink-queue=droptail --uplink-queue-args=packets=1 --downlink-queue=droptail --downlink-queue-args=packets=1" emu_1' % (emu_id, desc)
    procs.append(Popen(cmd, shell=True))

    emu_id = 'token-bucket based policer 2'
    desc = 'Token-bucket based policer (bandwidth 60mbps, RTT 20ms)'
    cmd = base_cmd + ' --id "%s" --desc "%s" --uplink-trace ~/traces/60mbps.trace --downlink-trace ~/traces/60mbps.trace --prepend-mm-cmds "mm-delay 10" --extra-mm-link-args "--uplink-queue=droptail --uplink-queue-args=packets=1 --downlink-queue=droptail --downlink-queue-args=packets=1" emu_2' % (emu_id, desc)
    procs.append(Popen(cmd, shell=True))

    emu_id = 'token-bucket based policer 3'
    desc = 'Token-bucket based policer (bandwidth 108mbps, RTT 20ms)'
    cmd = base_cmd + ' --id "%s" --desc "%s" --uplink-trace ~/traces/108mbps.trace --downlink-trace ~/traces/108mbps.trace --prepend-mm-cmds "mm-delay 10" --extra-mm-link-args "--uplink-queue=droptail --uplink-queue-args=packets=1 --downlink-queue=droptail --downlink-queue-args=packets=1" emu_3' % (emu_id, desc)
    procs.append(Popen(cmd, shell=True))

    emu_id = 'token-bucket based policer 4'
    desc = 'Token-bucket based policer (bandwidth 12mbps, RTT 100ms)'
    cmd = base_cmd + ' --id "%s" --desc "%s" --uplink-trace ~/traces/12mbps.trace --downlink-trace ~/traces/12mbps.trace --prepend-mm-cmds "mm-delay 50" --extra-mm-link-args "--uplink-queue=droptail --uplink-queue-args=packets=1 --downlink-queue=droptail --downlink-queue-args=packets=1" emu_4' % (emu_id, desc)
    procs.append(Popen(cmd, shell=True))

    emu_id = 'token-bucket based policer 5'
    desc = 'Token-bucket based policer (bandwidth 60mbps, RTT 100ms)'
    cmd = base_cmd + ' --id "%s" --desc "%s" --uplink-trace ~/traces/60mbps.trace --downlink-trace ~/traces/60mbps.trace --prepend-mm-cmds "mm-delay 50" --extra-mm-link-args "--uplink-queue=droptail --uplink-queue-args=packets=1 --downlink-queue=droptail --downlink-queue-args=packets=1" emu_5' % (emu_id, desc)
    procs.append(Popen(cmd, shell=True))

    emu_id = 'token-bucket based policer 6'
    desc = 'Token-bucket based policer (bandwidth 108mbps, RTT 100ms)'
    cmd = base_cmd + ' --id "%s" --desc "%s" --uplink-trace ~/traces/108mbps.trace --downlink-trace ~/traces/108mbps.trace --prepend-mm-cmds "mm-delay 50" --extra-mm-link-args "--uplink-queue=droptail --uplink-queue-args=packets=1 --downlink-queue=droptail --downlink-queue-args=packets=1" emu_6' % (emu_id, desc)
    procs.append(Popen(cmd, shell=True))
    ...
    cmd = base_cmd + ' --id "%s" --desc "%s" --uplink-trace ~/traces/12mbps.trace --downlink-trace ~/traces/0.12mbps.trace --prepend-mm-cmds "mm-delay 10" emu_1' % (emu_id, desc)
    procs.append(Popen(cmd, shell=True))

    emu_id = 'severe ACK aggregation 2'
    desc = 'Severe ACK aggregation (10 ACKs every 200ms)'
    cmd = base_cmd + ' --id "%s" --desc "%s" --uplink-trace ~/traces/12mbps.trace --downlink-trace ~/traces/10-every-200.trace --prepend-mm-cmds "mm-delay 10" emu_2' % (emu_id, desc)
    procs.append(Popen(cmd, shell=True))

    # scenario 3
    emu_id = 'bottleneck buffer 1'
    desc = 'Bottleneck buffer = BDP/10'
    cmd = base_cmd + ' --id "%s" --desc "%s" --uplink-trace ~/traces/12mbps.trace --downlink-trace ~/traces/12mbps.trace --prepend-mm-cmds "mm-delay 30" --extra-mm-link-args "--uplink-queue=droptail --uplink-queue-args=bytes=9000" emu_3' % (emu_id, desc)
    procs.append(Popen(cmd, shell=True))

    emu_id = 'bottleneck buffer 2'
    desc = 'Bottleneck buffer = BDP/3'
    cmd = base_cmd + ' --id "%s" --desc "%s" --uplink-trace ~/traces/12mbps.trace --downlink-trace ~/traces/12mbps.trace --prepend-mm-cmds "mm-delay 30" --extra-mm-link-args "--uplink-queue=droptail --uplink-queue-args=bytes=30000" emu_4' % (emu_id, desc)
    procs.append(Popen(cmd, shell=True))

    emu_id = 'bottleneck buffer 3'
    desc = 'Bottleneck buffer = BDP/2'
    cmd = base_cmd + ' --id "%s" --desc "%s" --uplink-trace ~/traces/12mbps.trace --downlink-trace ~/traces/12mbps.trace --prepend-mm-cmds "mm-delay 30" --extra-mm-link-args "--uplink-queue=droptail --uplink-queue-args=bytes=45000" emu_5' % (emu_id, desc)
    procs.append(Popen(cmd, shell=True))

    emu_id = 'bottleneck buffer 4'
    desc = 'Bottleneck buffer = BDP'
    cmd = base_cmd + ' --id "%s" --desc "%s" --uplink-trace ~/traces/12mbps.trace --downlink-trace ~/traces/12mbps.trace --prepend-mm-cmds "mm-delay 30" --extra-mm-link-args "--uplink-queue=droptail --uplink-queue-args=bytes=90000" emu_6' % (emu_id, desc)
    procs.append(Popen(cmd, shell=True))
